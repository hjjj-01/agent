...
这是一个员工工作量查询智能助手，整合了 WMS 实时数据和知识库检索。

=============================================================================
当前项目功能（精简版）：
=============================================================================
  - WMS 工具（1个）：查询员工工作量数据（工作数量、产出、重量、体积等）
  - RAG 工具（1个）：检索员工绩效考核、操作流程等知识内容
...
Agent的能力：
  - 查询员工工作量、产出、重量、体积等实时KPI数据（通过WMS工具）
  - 检索绩效考核标准、操作流程等知识（通过RAG工具）
  - 进行复杂的分析和推理（通过LLM）
  - 保持对话上下文，支持多轮对话
  - 自动决定调用哪些工具、调用几次
"""
...
class WMSAgent:
    ...
    一个智能的员工工作量查询助手，基于 LangChain 1.x 的 create_agent 构建。
    ...
    使用方式：
        ...
        response = agent.chat("查询今天的员工工作量")
    ...
    def __init__(self, config: Config):
        ...
        Args:
            config: 配置对象，包含 OpenAI API密钥、WMS接口地址、数据库路径等

        初始化流程（7步）：
            1. 创建LLM（大语言模型）——Agent的"大脑"
            2. 创建WMS客户端——获取实时员工工作量数据
            3. 创建RAG系统——检索知识库内容
            4. 构建知识库——加载示例文档
            5. 创建工具集——将功能包装成Agent可调用的工具
            6. 初始化对话消息历史——保持多轮对话上下文
            7. 创建Agent（create_agent）——LangChain 1.x 一站式创建

        说明：
            这个初始化过程将所有模块整合在一起，
            形成一个完整的智能对话系统。
            LangChain 1.x 的 create_agent 让创建步骤比 0.3.x 简化了很多。
        """
        ...
        # 第3步：创建RAG系统（检索增强生成）
        # =====================================================================
        # RAG（Retrieval-Augmented Generation）系统用于：
        #   - 存储操作手册、业务规则、产品信息等知识文档
        #   - 根据用户问题检索最相关的知识片段
        #   - 让 LLM 基于检索到的知识生成更准确的回答
        #
        # RAG 的工作原理：
        #   文档 → 分割成小块 → 向量化 → 存入向量数据库
        #   用户提问 → 向量化 → 搜索相似文档块 → 作为上下文给LLM
        ...
        # 第4步：构建知识库（使用示例数据）
        # =====================================================================
        # 示例数据包含 WMS 操作手册、业务规则、产品信息
        # 实际使用时可以替换为真实文档（使用 doc_processor.load_text_file 等）
        ...
        # 第5步：创建工具集
        # =====================================================================
        # 将 WMS 客户端和 RAG 系统的功能包装成 LangChain 工具
        #
        # 工具有两种类型：
        #   WMS工具（1个）：查询员工工作量（工作数量、产出、重量、体积等）
        #   RAG工具（1个）：知识库检索（考核标准、操作流程等）
        #
    ...
    def _create_system_prompt(self) -> str:
        ...
        """
        system_prompt = """
你是一个专业的WMS仓库员工工作量查询助手，能够帮助用户查询员工的工作量数据，
包括工作数量、产出数量、重量、体积等KPI信息，并提供知识查询服务。

你可以使用以下工具来完成用户的任务：
  - wms_query_employee_work：查询员工工作量数据
    · 重要：WMS API 只认ID（数字串），不认姓名/岗位名
    · 工具会自动查知识库，将员工姓名映射为worker_id，将岗位名映射为node_id
    · 用法：直接传 worker_name（如"张三"）或 node_name（如"打包"），工具自动查ID
    · 知识库存放了完整的员工姓名→ID 和岗位名→ID 映射表
  - rag_search_knowledge：检索知识库
    · 用于查找员工ID、岗位ID，或查询操作流程、绩效考核标准等知识

回复规则：
  1. 查询具体员工的工作量（如"张三的工作量"）：传 worker_name 参数，工具自动查知识库找ID
  2. 查询某岗位的工作量（如"打包岗位的工作量"）：传 node_name 参数，工具自动查知识库找ID
  3. 如果用户只说"工作量"：工具会查默认岗位所有人的数据
  4. 如果查询结果为空，告诉用户"在知识库中找到了ID，但在当天工作量数据中没有记录"
  5. 回答要专业、清晰，使用中文，适当使用表格展示数据
"""
